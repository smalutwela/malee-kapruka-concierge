...
| `lib/account/store.ts` + `account/capture.ts` | Persisted signed-in email + cached display name (`malee-account`), plus the exported `DEMO_EMAIL` (the one address the preview backend serves — single source of truth, not repeated per locale); `useCaptureAccount` caches the name from a profile lookup and **fills blank** checkout details from the account (never overwrites the shopper's own edits) |
...
| `components/account-drawer.tsx` | Kapruka **sign-in** (email the shopper types) + a one-tap **"Try the demo account"** button — the preview serves one address, so that is the path that actually works; it still routes through the same `submit()`, so the email reaches the allowlist as shopper-supplied. Plus the saved-details editor + local order history with one-tap **Reorder** / Pay / Track |
...
- **The header account control** (`AccountButton`) is one button with two faces: signed out it is a filled **"Sign in"** pill whose label survives every breakpoint (the word is the whole point — top-right is where people scan for auth); signed in it becomes the shopper's initial, and the "Orders" label returns from `sm` up. Both open the same sheet, so the header still holds three controls and fits 375px in all three locales (Sinhala is the widest at ~126px).
