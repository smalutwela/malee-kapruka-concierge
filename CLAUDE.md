...
| `lib/agent/tools.ts` | 9 curated tools (clean Zod schemas → MCP `params` envelope). **Search results are private to the model** — only `presentProducts` (model-picked ids, served from a per-instance result stash with a `get_product` fallback) renders product cards. `addToCart` resolves the item server-side (stash → cached `get_product` — never model-typed data) and returns it with a unique `addId` |
